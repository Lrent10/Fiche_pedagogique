...
export type SheetDetail = {
  ...
  flow: { id: number; block_instance_id: number; phase_code: string; teacher_action: string; learner_action: string; strategy: string; duration_minutes: number; position: number }[]
}
...
export type SupportDetail = { id: number; support_id: number; code: string; title: string; revision_number: number; status: string; resources: { id: number; origin: string; title: string; blocks: Block[] }[] }
