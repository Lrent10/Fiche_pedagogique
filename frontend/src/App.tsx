      ...
      <main>
        ...
        {loading ? <div className="empty">Chargement de l’espace de travail…</div> : (
          <div className="page">
            ...
            {view === 'supports' && (activeSupport ? <SupportEditor support={activeSupport} setSupport={setActiveSupport} resources={resources} notify={showNotice} refresh={refresh} close={() => setActiveSupport(null)} /> : <SupportsPage supports={supports} onOpen={openSupport} onCreated={(support) => { setActiveSupport(support); void refresh() }} />)}
...
function SheetEditor({ sheet, setSheet, resources, notify, refresh, close }: { sheet: SheetDetail; setSheet: (s: SheetDetail) => void; instructions: Instruction[]; resources: Resource[]; notify: (m: string) => void; refresh: () => Promise<void>; close: () => void }) {
  ...
  return <>
    ...
    <section className="editor-panel">
      ...
      {step === 4 && <div><SectionTitle number="05" title="Organiser le déroulement" subtitle="Réordonnez les blocs selon la progression de la séance." />{sheet.flow.map((flow, index) => { const block = blocks.find(b => b.id === flow.block_instance_id); return <div className="flow-row" key={flow.id}><span className="drag">⋮⋮</span><span className="flow-index">{index + 1}</span><div><b>{block?.title || 'Bloc'}</b><LatexPreview content={block?.content_latex || ''} compact /></div><span>{flow.duration_minutes} min</span>{draft && <div className="move"><button onClick={() => move(flow.block_instance_id, -1)}>↑</button><button onClick={() => move(flow.block_instance_id, 1)}>↓</button></div>}</div> })}</div>}
...
function SupportEditor({ support, setSupport, resources, notify, refresh, close }: { support: SupportDetail; setSupport: (s: SupportDetail) => void; resources: Resource[]; notify: (m: string) => void; refresh: () => Promise<void>; close: () => void }) { const draft = support.status === 'DRAFT'; async function add(id: number) { setSupport(await post<SupportDetail>(`/api/supports/${support.id}/resources/library`, { resource_version_id: id })); notify('Activité ajoutée au support.') } async function toggle(id: number, visible: boolean) { setSupport(await put<SupportDetail>(`/api/supports/${support.id}/blocks/${id}`, { visible })); } async function finalize() { setSupport(await post<SupportDetail>(`/api/supports/${support.id}/finalize`)); await refresh(); notify('Support finalisé.') } async function revise() { setSupport(await post<SupportDetail>(`/api/supports/${support.id}/new-revision`)); await refresh() } async function exportPdf(target: 'LEARNER_INITIAL' | 'LEARNER_COMPLETED') { const result = await post<{ download_url: string }>('/api/exports', { document_family: 'LEARNER', revision_id: support.id, target }); window.open(result.download_url, '_blank'); notify(target === 'LEARNER_INITIAL' ? 'PDF initial généré.' : 'PDF complété généré.') } return <><button className="back" onClick={close}>← Retour aux supports</button><div className="editor-head"><div><div className="meta-line"><Status value={support.status} /><span>{support.code} · Révision {support.revision_number}</span></div><h2>{support.title}</h2></div>{!draft && <button className="primary" onClick={revise}>Nouvelle révision</button>}</div><div className="two-columns"><section className="panel"><h3>Contenu du support</h3>{support.resources.length === 0 && <div className="empty small">Ajoutez une activité depuis la colonne de droite.</div>}{support.resources.map(r => <article className="support-resource" key={r.id}><h4>{r.title}</h4>{r.blocks.map(b => <div className={b.visible === false ? 'hidden-block' : ''} key={b.id}><div className="block-head"><b>{b.title}</b>{draft && <label className="switch">Visible <input type="checkbox" checked={b.visible !== false} onChange={e => toggle(b.id, e.target.checked)} /></label>}</div>{b.visible !== false && <LatexPreview content={b.content_latex} />}</div>)}</article>)}<div className="final-actions">{draft && <button className="primary" onClick={finalize}>Finaliser</button>}<button className="outline" onClick={() => exportPdf('LEARNER_INITIAL')}>PDF initial</button><button className="outline" onClick={() => exportPdf('LEARNER_COMPLETED')}>PDF complété</button></div></section><aside className="panel picker"><h3>Ajouter une activité</h3>{resources.map(r => <button key={r.id} disabled={!draft} onClick={() => add(r.id)}><span>{r.provenance_kind === 'SOURCED' ? 'Guide' : 'DÉMO'}</span><b>{r.title}</b><small>＋ Ajouter</small></button>)}</aside></div></> }
